  ...
  const loadData = async () => {
    try {
      ...
      const [messagesResponse, announcementsResponse, analyticsResponse] = await Promise.all([
        fetch('http://localhost:9001/api/admin/communications/messages/sent', {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
        }),
        fetch('http://localhost:9001/api/admin/communications/announcements', {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
        }),
        fetch('http://localhost:9001/api/admin/communications/analytics/overview', {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-1 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="text-green-600 dark:text-green-400">{message.delivery_stats.delivered}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600 dark:text-gray-400">Read:</span>
                          <span className="text-blue-600 dark:text-blue-400">{message.delivery_stats.read}</span>
                        </div>
                        {message.delivery_stats.failed > 0 && (
                          <div className="flex justify-between">
                            <span className="text-gray-600 dark:text-gray-400">Failed:</span>
                            <span className="text-red-600 dark:text-red-400">{message.delivery_stats.failed}</span>
                          </div>
                        )}
                      </div>
                    </td>
              ...
              {Object.entries(analytics.delivery_performance).map(([channel, stats]) => (
                <div key={channel} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Delivered:</span>
                      <span className="text-green-600 dark:text-green-400">{stats.delivered}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Read:</span>
                      <span className="text-blue-600 dark:text-blue-400">{stats.read}</span>
                    </div>
